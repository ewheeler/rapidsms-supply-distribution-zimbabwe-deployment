...
class HeadmasterOrDEO(models.Model):
    ...
    def closest_by_name(klass, search_string, n=100):
        ...
        for contact in klass.objects.all():
            # guess contact's surname: separate the biggest word in their full name
            surname_guess = max(str(contact.name).replace('.', ' ').split())

            args = [str(contact.name).upper(), search_string.upper()]
            alt_args = [surname_guess.upper(), search_string.upper()]
            ...
            d.append((search_string, contact,\
